...
directory does a lot of that for you. You would run it like so:
    ...
    go install github.com/corbado/oapi-codegen/cmd/oapi-codegen@latest
...
import (
	...
	"github.com/corbado/oapi-codegen/examples/petstore-expanded/gin/api"
	middleware "github.com/corbado/oapi-codegen/pkg/gin-middleware"
)
...
type Client struct {
    ...
    // https://api.corbado.com for example.
    Server string
...
```go
    import (
        "github.com/corbado/oapi-codegen/pkg/securityprovider"
    )
    ...
    func CreateSampleProviders() error {
        ...
        //
        client, clientErr := NewClient("https://api.corbado.com", WithRequestEditorFn(apiKeyProvider.Intercept))
...
[`/internal/test/externalref/`](https://github.com/corbado/oapi-codegen/blob/master/internal/test/externalref/externalref.cfg.yaml)
...
output-options:
  skip-prune: true
import-mapping:
  ./packageA/spec.yaml: github.com/corbado/oapi-codegen/internal/test/externalref/packageA
  ./packageB/spec.yaml: github.com/corbado/oapi-codegen/internal/test/externalref/packageB
output: externalref.gen.go
output-options:
  skip-prune: true
```

Have a look at [`cmd/oapi-codegen/oapi-codegen.go`](https://github.com/corbado/oapi-codegen/blob/master/cmd/oapi-codegen/oapi-codegen.go#L48) 
...
let's call it `github.com/corbado/some-package`. You need to tell `oapi-codegen` that
`some_spec.yaml` corresponds to this package, and you would do it by specifying
this command line argument:

    -import-mapping=./some_spec.yaml:github.com/corbado/some-package

This tells us that in order to resolve references generated from `some_spec.yaml` we
need to import `github.com/corbado/some-package`. You may specify multiple mappings
